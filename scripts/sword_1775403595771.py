...
# Create fuller indentation
for v in blade.data.vertices:
    dist_from_center = abs(v.co.y)
    if dist_from_center < 0.05 and v.co.z > 0.2 and v.co.z < 1.6:
        v.co.x *= 0.85
    if v.co.z > 1.75:
        taper = (v.co.z - 1.75) / 0.25
        v.co.x *= (1.0 - taper * 0.95)
        v.co.y *= (1.0 - taper * 0.85)
    elif v.co.z > 1.3:
        factor = (v.co.z - 1.3) / 0.45
        v.co.x *= (1.0 - factor * 0.4)
...
output_path = "C:/Users/Utilisateur/Desktop/FabWare/MeshyMyself/meshes/sword_1775403595771.glb"
