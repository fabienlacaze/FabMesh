...
def from_pretrained(path: str, **kwargs):
    ...
    if is_local:
        ...
        # Patched: pipeline.json sometimes stores entries like
        # `ckpts/shape_dec_next_dc_f16c32_fp16` (relative to TRELLIS.2-4B)
        # alongside `microsoft/TRELLIS-image-large/ckpts/...` (cross-repo).
        # Original code assumed always 3+ parts; the 2-part case produced
        # repo_id="ckpts/foo" + model_name="" -> 404.
        if len(path_parts) >= 3:
            repo_id = f'{path_parts[0]}/{path_parts[1]}'
            model_name = '/'.join(path_parts[2:])
        else:
            # Relative path -> default to the parent TRELLIS.2-4B repo.
