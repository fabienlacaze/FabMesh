...
def main():
    if len(sys.argv) < 6:
        print('Usage: mesh_pre_transform.py <in.glb> <out.glb> <tx> <ty> <scale>')
        sys.exit(1)
    in_glb = sys.argv[1]
    out_glb = sys.argv[2]
    tx = float(sys.argv[3])
    ...
    print(f'[pre_transform] in={in_glb} out={out_glb} tx={tx} ty={ty} scale={scale}',
          flush=True)
