...
def run_rig(
    ...
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        try:
            ...
            # A mesh SkinTokens can't rig (e.g. a mechanical object with no natural
            # skeleton) raises here; skip it so the rest of the batch still runs.
            print(f"[SKIP] item {i} ({output_paths[i].name}) failed: {type(_e).__name__}: {_e}")
            continue
...
def launch_gradio():
    # Lazy import: gradio pulls pandas, whose index.cp311-win_amd64.pyd is
    # blocked by Windows Smart App Control on this machine. The CLI rigging
    # path never touches gradio/pandas, so import it only in web-UI mode.
    import gradio as gr
