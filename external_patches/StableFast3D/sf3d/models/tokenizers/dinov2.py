...
# Shim: find_pruneable_heads_and_indices was removed from
# transformers.pytorch_utils in transformers 5.x. FabMesh runs transformers
# 5.x for SDXL/Juggernaut, so we reimplement the tiny helper locally.
# Source: transformers 4.42 pytorch_utils.py (Apache 2.0).
...
class Dinov2Model(Dinov2PreTrainedModel):
    ...
    def forward(
        ...
        # Prepare head mask if needed. transformers 5.x removed
        # `PreTrainedModel.get_head_mask`, and SF3D never passes one in
        # practice, so expand None -> [None] * num_hidden_layers inline
        # (equivalent to the old get_head_mask behaviour for head_mask=None).
