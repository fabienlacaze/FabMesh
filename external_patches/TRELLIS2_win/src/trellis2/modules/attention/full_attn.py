...
def scaled_dot_product_attention(*args, **kwargs):
    ...
    if config.BACKEND == 'xformers':
        ...
        k = k.permute(0, 2, 1, 3)   # [N, H, L, C]
        v = v.permute(0, 2, 1, 3)   # [N, H, L, C]
        # [blackwell_fix] Force math backend on Blackwell — flash/efficient
        # backends produce numerically broken outputs on sm_120 with bf16
        # (causes Y-axis collapse in sparse_structure_decoder occupancy map).
        try:
