...
def sparse_scaled_dot_product_attention(*args, **kwargs):
    ...
    if config.ATTN == 'xformers':
        ...
        # [blackwell_fix] SDPA fallback for sm_120 — flash_attn bf16 produces
        # noise on RTX 50xx Blackwell. Process each sequence separately with
        # PyTorch SDPA in fp32 (correct on Blackwell).
        # CRITICAL FIX (v2): Force fp32 attention math + .contiguous() before
        # SDPA. On sm_120, bf16 with large seqlen (>4k) produces silent
        # precision loss in softmax, destroying spatial coherence.
        # FABMESH PERF FIX (2026-06-14): prefer the EFFICIENT (mem-efficient)
        # kernel over MATH. MATH materializes the full N*N score matrix — on a
        # large voxel sequence it tries to allocate 16-64 GB and OOMs / thrashes
        # to host RAM (the real cause of the ~8-min SLat stall). EFFICIENT
        # streams the attention (O(N) memory) and is BIT-IDENTICAL to MATH in
        # fp32 (benchmarked maxerr=0.0000 on this RTX 5080) and 2-3x faster.
        # MATH stays as fallback if EFFICIENT is unavailable for some shape.
        ...
        try:
            ...
            import contextlib
            _kernel_ctx = contextlib.nullcontext()
        if num_all_args == 1:
            q_all, k_all, v_all = qkv.unbind(dim=1)  # each [T, H, C]
        elif num_all_args == 2:
            k_all, v_all = kv.unbind(dim=1)
            q_all = q
        else:
        ...
        with _kernel_ctx:
            for ql, kl in zip(q_seqlen, kv_seqlen):
                # [1, H, ql, C] CONTIGUOUS in fp32 to avoid Blackwell sm_120
                # bf16 softmax precision loss with large seqlens
                qi = q_all[q_offset:q_offset+ql].permute(1, 0, 2).unsqueeze(0).contiguous().float()
                ki = k_all[kv_offset:kv_offset+kl].permute(1, 0, 2).unsqueeze(0).contiguous().float()
                vi = v_all[kv_offset:kv_offset+kl].permute(1, 0, 2).unsqueeze(0).contiguous().float()
                oi = _sdpa(qi, ki, vi)  # [1, H, ql, C] in fp32
                # Back to [ql, H, C] in original dtype
                oi = oi.squeeze(0).permute(1, 0, 2).contiguous().to(orig_dtype)
                out_list.append(oi)
                q_offset += ql
                kv_offset += kl
