...
def patch_all(force: bool = False, verbose: bool = True):
    ...
    # Disable spconv/cumm NVRTC JIT — sm_120 NVRTC paths produce silent
    # garbage output (CUTLASS grouped GEMM bug, NVIDIA/cutlass#3096; also
    # confirmed by visualbruno/ComfyUI-Trellis2#157). With JIT disabled
    # spconv falls back to the precompiled sm_86 PTX from the wheel,
    # which JIT-loads cleanly to sm_120 via the CUDA driver.
    os.environ["SPCONV_DISABLE_JIT"] = "1"
    os.environ["CUMM_DISABLE_JIT"] = "1"

    # ── 1) Patch torch.cuda.get_device_capability ──
    # Spoof to (8, 6) Ampere — NOT (9, 0) Hopper. Hopper sm_90 PTX uses
    # Hopper-only ops (TMA, WGMMA, thread-block clusters) that are NOT
    # available on consumer Blackwell sm_120. Ampere sm_86 PTX is the
    # newest arch whose PTX has zero Hopper/Blackwell-only instructions,
    # so it JIT-compiles cleanly to sm_120 via the CUDA driver.
    # This is the same target visualbruno/ComfyUI-Trellis2 uses on RTX 50xx.
    _orig_cap = torch.cuda.get_device_capability
    def _patched_cap(device=None):
        m, n = _orig_cap(device)
        return (8, 6) if m >= 10 else (m, n)
    ...
    try:
        ...
        if verbose:
            print("[blackwell_fix] Patched cumm CC detection -> (8, 6) Ampere")
    except ImportError:
        pass
