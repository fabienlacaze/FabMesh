...
### Diag analysis of the first "fixed" run

After fixing the bouche-au-mollet bug by setting
`FABMESH_SF3D_NORMALIZE_ORIENT=0` (which disables the +180° Y rotation
and so removes the offset bug), the texture **placement** was correct
but the **detail** was visibly worse than the original buggy run —
face went cireux, denim creases lost, baskets soft. Diag maps showed:

- 40% of atlas texels **uncovered** (push-pull fills with a blur).
- Only 9k texels with single-view coverage (~1%, fragile).
- 612k with 2+ views (~58%) — but most of that is dup contribution,
  not real new info.
- The native HD `input.png` (1151px, prio 0.7) was *systematically
  outranked* by `mv/view_0.png` (1024px dup, prio 1.0). The pipeline
  was throwing away the highest-quality signal in favour of its own
  resized copy.

Root cause of the detail loss: with NORMALIZE_ORIENT=0, the mesh stays
in SF3D's native frame (face -> -Z) but `texture_project`'s built-in
V/U flips and visibility math assume the post-rotation frame
(face -> +Z, three.js convention). Disagreement → fewer verts pass
the visibility test → more atlas holes → more push-pull blur. So the
"orientation fix" was actually a **double bug fix**: it solved the
mouth-on-calf placement but caused a visibility-collapse blur.

### Real fix (final)

Reverted to `NORMALIZE_ORIENT=1` (default) and added a new env flag
`FABMESH_TEXPROJ_SHIFT_SOURCE=1` that tells texture_project to apply
`rotation_offset_deg` to the source `input.png` azimuth too — so that
both the source image AND every multiview slot live in the same
post-rotation frame. `ip45_2view_to_3d.py` exports it.

After this fix, projecting log shows:
```
projecting 7 view(s):
  az=180/el=0(p=0.7),   <- input.png (was at 0, now shifted +180)
  az=180/el=0(p=1.0),   <- mv/0 dup front
  az=270/el=0(p=0.9),   <- mv/1 dup front (right slot)
  az=0/el=0(p=0.8),     <- mv/2 back
  az=90/el=0(p=0.9),    <- mv/3 dup back (left slot)
  az=180/el=90(p=0.7),
  az=180/el=-90(p=0.7)
```

Visual: confirmed by user — "ce résultat à été le meilleur jusqu'à
présent en terme de placement". Face/denim/shorts/baskets all in
correct anatomical positions, with detail equivalent to the buggy
first run.

### Remaining issues (low priority for now)

- 40% atlas holes still — UV-island packing leaves big black gaps
  that push-pull guesses. Would need a denser xatlas re-pack OR a
  texture target other than 1024 (counter-intuitively, raising
  tex_res doesn't help because xatlas islands stay small).
- input.png at prio 0.7 still outranked by mv/0 dup at 1.0 → we
  bleed resolution. Worth bumping input.png to 1.0 and the dup-front
  slots to 0.5 in PRIORITY_WEIGHTS.
- No real left/right profile views — sides are dup-back/dup-front.
  Easy win: feed `images/child/_scale_sweep/ip35_right.png` and
  `ip35_left.png` (already generated) into mv/view_1 and mv/view_3.
- `FABMESH_TEXPROJ_SHIFT_SOURCE` is opt-in — for the existing
  Z123/CRM pipelines the flag stays off, so this commit is
  backward-compatible. Worth re-evaluating whether the "shift source
  too" should become the default once we have an end-to-end harness.
