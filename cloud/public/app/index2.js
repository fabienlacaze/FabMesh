...
const ASSET_TYPE_PROMPTS = {
  ...
  animal: 'wildlife photography of ONE animal only, single creature, full body lateral profile, all four feet flat on the ground, body horizontal parallel to floor, belly close to ground, head and tail at the same low height as the spine, tail extending behind, four legs supporting the body from below, low camera angle eye-level with the animal, plain white background, even studio lighting, no shadows, NEVER bipedal, NEVER upright, NEVER standing on hind legs, NEVER humanoid posture, NEVER T-pose, NEVER cartoon mascot stance, no humanoid anthropomorphism, no second animal, no duplicate, no text, no UI',
  custom: '',
};
